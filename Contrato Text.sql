SELECT
  ...
  (tab1.VALOR_TOTAL_PEDIDO - tab1.valor_saldo_pedido) AS SALDO_RESTANTE,
  tab1.STATUS_PEDIDO,
  tab1.COMPRADOR,
  tab1.DATA_PEDIDO AS INCIO_VIGENCIA,
  tab1.validade_contrato AS FIM_VIGENCIA,
  tab1.FORN_COD,
  tab1.FORN_NOME,
  tab1.FORN_CNPJ,
  tab1.REQUISITANTE,
...
FROM
  (
    SELECT
      to_date (ek.KDATE, 'yyyymmdd') AS validade_contrato,
      ek.BSART,
      CASE aprov.seq_hist
    ...
    WHERE
      ek.MANDT = '300'
      --AND ep.konnr = '{{$.trigger.headers.pedidoORcontrato}}' --Contrato (com historico)
      AND ek.ebeln = '{{$.trigger.headers.pedidoORcontrato}}'
      AND ep.EBELP = '00010'
      --AND  = 'WK'
  ) tab1
GROUP by
